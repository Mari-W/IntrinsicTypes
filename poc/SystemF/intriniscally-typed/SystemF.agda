...
-- 
-- abc : (T T⋯ₛ ((ρ★ T≫ᵣₛ Tidₛ) T↑ₛ ℓ)) ≡ (T T⋯ᵣ (ρ★ T↑ᵣ ℓ))
-- abc {ℓ = ℓ} {T = T} {ρ★ = ρ★} = {! refl !}
-- 
-- E↑ℓcoincidence : (ℓ : Level) (ρ★ : Δ₁ T⇛ᵣ Δ₂) (ρ : Γ₁ E⇛ᵣ[ ρ★ ] Γ₂) → 
--   [ _ ، Tidₛ T↑ₛ ℓ ] ([ _ ] ρ Eℓ↑ᵣ ℓ) E≫ᵣₛ Eidₛ ≡ 
--     subst (λ σ★ → (Γ₁ ،★ ℓ) E⇛ₛ[ {! σ★  !} ] (Γ₂ ،★ ℓ)) (T↑coincidence ρ★ ℓ) ([ ρ★ T≫ᵣₛ Tidₛ ] ([ ρ★ ، Tidₛ ] ρ E≫ᵣₛ Eidₛ) Eℓ↑ₛ ℓ)
...
ρ★⟦_⟧_ {Δ₁ = []}    ρ★ η✦₂ = []
ρ★⟦_⟧_ {Δ₁ = _ ∷ _} ρ★ η✦₂ = T⟦ ` ρ★ _ (here refl) ⟧ η✦₂ ∷ ρ★⟦ (λ ℓ z → ρ★ ℓ (there z)) ⟧ η✦₂

⟦ρ★⟧-preserves-wk : (ρ★ : Δ₁ T⇛ᵣ Δ₂) → (⟦T⟧ : Set ℓ) → (η✦₂ : Env✦ Δ₂) → 
  (ρ★⟦ ρ★ T≫ᵣᵣ Twkᵣ ⟧ (⟦T⟧ ∷ η✦₂)) ≡ω (ρ★⟦ ρ★ ⟧ η✦₂)
⟦ρ★⟧-preserves-wk {Δ₁ = []}     ρ★ ⟦T⟧ η✦₂ = refl
⟦ρ★⟧-preserves-wk {Δ₁ = l ∷ Δ₁} ρ★ ⟦T⟧ η✦₂ = 
  cong-ωω (lookup η✦₂ (ρ★ l (here refl)) ∷_) (⟦ρ★⟧-preserves-wk (λ ℓ z → ρ★ ℓ (there z)) ⟦T⟧ η✦₂)
...
⟦ρ★⟧-preserves-lookup (there x) ρ★ η✦   rewrite ⟦ρ★⟧-preserves-lookup x (λ ℓ z → ρ★ ℓ (there z)) η✦ = refl
...
σ★⟦_⟧_ {Δ₁ = []}    σ★ η✦₂ = []
σ★⟦_⟧_ {Δ₁ = _ ∷ _} σ★ η✦₂ = T⟦ σ★ _ (here refl) ⟧ η✦₂ ∷ σ★⟦ (λ ℓ z → σ★ ℓ (there z)) ⟧ η✦₂

Tsemantic-coincidence : (ρ★ : Δ₁ T⇛ᵣ Δ₂) (η✦₂ : Env✦ Δ₂) → (σ★⟦ ρ★ T≫ᵣₛ Tidₛ ⟧ η✦₂) ≡ω (ρ★⟦ ρ★ ⟧ η✦₂)
Tsemantic-coincidence {[]}     ρ★ η✦₂ = refl
Tsemantic-coincidence {x ∷ Δ₁} ρ★ η✦₂ = cong-ωω (lookup η✦₂ (ρ★ x (here refl)) ∷_) (Tsemantic-coincidence {Δ₁ = Δ₁} (λ ℓ z → ρ★ ℓ (there z)) η✦₂)
...
⟦σ★⟧-preserves-wk {Δ₁ = l ∷ Δ₁} σ★ ⟦T⟧ η✦₂ = cong-ωω ((T⟦ (σ★ _ (here refl)) ⟧ η✦₂) ∷_) (⟦σ★⟧-preserves-wk (λ ℓ z → σ★ ℓ (there z)) ⟦T⟧ η✦₂)
...
⟦σ★⟧-preserves-lookup (there x)   σ★ η₂ = ⟦σ★⟧-preserves-lookup x (λ ℓ z → σ★ ℓ (there z)) η₂
...
ρ⟦_⟧_ : ∀ {Δ₁} {Γ₁ : Ctx Δ₁} {Γ₂ : Ctx Δ₂} {η✦₁ : Env✦ Δ₁} {η✦₂ : Env✦ Δ₂} {ρ★ : Δ₁ T⇛ᵣ Δ₂} →
  Γ₁ E⇛ᵣ[ ρ★ ] Γ₂ → Ctx✦ Γ₁ η✦₁ → Ctx✦ Γ₂ η✦₂
ρ⟦_⟧_ {Γ₁ = ∅}        ρ Γ✦₂ = λ ℓ T x → {! Δ₁ !}
ρ⟦_⟧_ {Γ₁ = Γ₁ ، T}   ρ Γ✦₂ = ρ⟦ (λ ℓ T x → ρ ℓ T (there x)) ⟧ (λ ℓ T x → Γ✦₂ ℓ T (there x))
ρ⟦_⟧_ {Γ₁ = Γ₁ ،★ ℓ}  ρ Γ✦₂ = {!   !}
