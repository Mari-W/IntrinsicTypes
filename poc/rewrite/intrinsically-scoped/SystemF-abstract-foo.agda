{-# OPTIONS --rewriting #-}
module SystemF-abstract-foo where
...
abstract
   ...
   postulate
      ...
      -- Forward Composition Primitves
      ≫ᵣᵣ-def₂           : (ρ₁ : S₁ ⇛ᵣ S₂) (ρ₂ : S₂ ⇛ᵣ S₃) (T : S₁ ⊢ s) → T ⋯ᵣ (ρ₁ ≫ᵣᵣ ρ₂) ≡ T ⋯ᵣ λ _ x → ρ₂ _ (ρ₁ _ x)
      ≫ᵣₛ-def₂           : (ρ₁ : S₁ ⇛ᵣ S₂) (σ₂ : S₂ ⇛ₛ S₃) (T : S₁ ⊢ s) → T ⋯ₛ (ρ₁ ≫ᵣₛ σ₂) ≡ T ⋯ₛ λ _ x → σ₂ _ (ρ₁ _ x)
      ≫ₛᵣ-def₂           : (σ₁ : S₁ ⇛ₛ S₂) (ρ₂ : S₂ ⇛ᵣ S₃) (T : S₁ ⊢ s) → T ⋯ₛ (σ₁ ≫ₛᵣ ρ₂) ≡ T ⋯ₛ λ _ x → (σ₁ _ x) ⋯ᵣ ρ₂
      ≫ₛₛ-def₂           : (σ₁ : S₁ ⇛ₛ S₂) (σ₂ : S₂ ⇛ₛ S₃) (T : S₁ ⊢ s) → T ⋯ₛ (σ₁ ≫ₛₛ σ₂) ≡ T ⋯ₛ λ _ x → (σ₁ _ x) ⋯ₛ σ₂
      ...
      -- Coinicidence Law
      coincidence        : (ρ : S₁ ⇛ᵣ S₂) (T : S₁ ⊢ s) → T ⋯ₛ (λ _ x → ` ρ _ x) ≡ T ⋯ᵣ ρ
