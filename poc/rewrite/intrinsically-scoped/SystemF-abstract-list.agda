{-# OPTIONS --rewriting #-}
module SystemF-abstract-list where
...
-- lookup' : ∀ {A : Set} {P : A → Set} {xs : List A} → All P xs → (∀ {x} → x ∈ xs → P x)
-- lookup' (px ∷ xs) (here refl) = px
-- lookup' (px ∷ xs) (there x) = lookup' xs x
postulate
  lookup : ∀ {ℓ ℓ'} {A : Set ℓ} {P : A → Set ℓ'} {xs : List A} → All P xs → (∀ {x} → x ∈ xs → P x)
  ...
  _⋯ᵣ_ : S₁ ⊢ s → S₁ ⇛ᵣ S₂ → S₂ ⊢ s
  -- (` x)         ⋯ᵣ ρ = ` (ρ _ x)
  -- (λx e)        ⋯ᵣ ρ = λx (e ⋯ᵣ (here refl ∷ (ρ ≫ᵣᵣ wkᵣ)))
  -- (Λα e)        ⋯ᵣ ρ = Λα (e ⋯ᵣ (here refl ∷ (ρ ≫ᵣᵣ wkᵣ)))
  -- (∀[α∶ k ] t)  ⋯ᵣ ρ = ∀[α∶ k ⋯ᵣ ρ ] (t ⋯ᵣ (here refl ∷ (ρ ≫ᵣᵣ wkᵣ)))
  -- (e₁ · e₂)     ⋯ᵣ ρ = (e₁ ⋯ᵣ ρ) · (e₂ ⋯ᵣ ρ)
  -- (e ∙ t)       ⋯ᵣ ρ = (e ⋯ᵣ ρ) ∙ (t ⋯ᵣ ρ)
  -- (t₁ ⇒ t₂)     ⋯ᵣ ρ = (t₁ ⋯ᵣ ρ) ⇒ (t₂ ⋯ᵣ ρ)
  -- ★             ⋯ᵣ ρ = ★ 
  ...
  _⋯ₛ_ : S₁ ⊢ s → S₁ ⇛ₛ S₂ → S₂ ⊢ s
  -- (` x)         ⋯ₛ σ = (σ _ x)
  -- (λx e)        ⋯ₛ σ = λx (e ⋯ₛ ((` here refl) ∷ (σ ≫ₛᵣ wkᵣ)))
  -- (Λα e)        ⋯ₛ σ = Λα (e ⋯ₛ ((` here refl) ∷ (σ ≫ₛᵣ wkᵣ)))
  -- (∀[α∶ k ] t)  ⋯ₛ σ = ∀[α∶ k ⋯ₛ σ ] (t ⋯ₛ ((` here refl) ∷ (σ ≫ₛᵣ wkᵣ)))
  -- (e₁ · e₂)     ⋯ₛ σ = (e₁ ⋯ₛ σ) · (e₂ ⋯ₛ σ)
  -- (e ∙ t)       ⋯ₛ σ = (e ⋯ₛ σ) ∙ (t ⋯ₛ σ)
  -- (t₁ ⇒ t₂)     ⋯ₛ σ = (t₁ ⋯ₛ σ) ⇒ (t₂ ⋯ₛ σ)
  -- ★             ⋯ₛ σ = ★ 
...
postulate
  -- Lookup Primitives
  lookup-def₁ : ∀ {ℓ ℓ'} {A : Set ℓ} {P : A → Set ℓ'} {xs : List A} (x : A) (px : P x) (pxs : All P xs) → lookup (px ∷ pxs) (here refl) ≡ px
  lookup-def₂ : ∀ {ℓ ℓ'} {A : Set ℓ} {P : A → Set ℓ'} {xs : List A} (x : A) (x' : A) (xx : x' ∈ xs) (px : P x) (pxs : All P xs) → lookup (px ∷ pxs) (there xx) ≡ lookup pxs xx

  -- Renaming Primitives
  idᵣ-def            : (x : s ∈ S) → lookup idᵣ x ≡ x
  wkᵣ-def            : (x : s ∈ S) → lookup (wkᵣ {s = s'}) x ≡ there x
  ∷ᵣ-def             : (x : s ∈ S₂) (ρ : S₁ ⇛ᵣ S₂) → x ∷ᵣ ρ ≡ x ∷ ρ
  
  -- Renaming Instantiation Primitves
  `_⋯ᵣ_              : (x : s ∈ S₁) (ρ : S₁ ⇛ᵣ S₂) → (` x) ⋯ᵣ ρ ≡ ` lookup ρ x 
  ...
  idₛ-def            : (x : s ∈ S) → lookup idₛ x ≡ ` x
  ∷ₛ-def             : (T : S₂ ⊢ s)  (σ : S₁ ⇛ₛ S₂) → T ∷ₛ σ ≡ T ∷ σ

  -- Substitution Instantiation Primitves
  `_⋯ₛ_              : (x : s ∈ S₁) (σ : S₁ ⇛ₛ S₂) → (` x) ⋯ₛ σ ≡ lookup σ x 
  ...
  ≫ᵣᵣ-def            : (ρ₁ : S₁ ⇛ᵣ S₂) (ρ₂ : S₂ ⇛ᵣ S₃) (x : s ∈ S₁) → lookup (ρ₁ ≫ᵣᵣ ρ₂) x ≡ lookup ρ₂ (lookup ρ₁ x)
  ≫ᵣₛ-def            : (ρ₁ : S₁ ⇛ᵣ S₂) (σ₂ : S₂ ⇛ₛ S₃) (x : s ∈ S₁) → lookup (ρ₁ ≫ᵣₛ σ₂) x ≡ lookup σ₂ (lookup ρ₁ x)
  ≫ₛᵣ-def            : (σ₁ : S₁ ⇛ₛ S₂) (ρ₂ : S₂ ⇛ᵣ S₃) (x : s ∈ S₁) → lookup (σ₁ ≫ₛᵣ ρ₂) x ≡ (lookup σ₁ x) ⋯ᵣ ρ₂
  ≫ₛₛ-def            : (σ₁ : S₁ ⇛ₛ S₂) (σ₂ : S₂ ⇛ₛ S₃) (x : s ∈ S₁) → lookup (σ₁ ≫ₛₛ σ₂) x ≡ (lookup σ₁ x) ⋯ₛ σ₂
  ...
  η-lawᵣ             : (ρ : (s ∷ S₁) ⇛ᵣ S₂) → _∷ᵣ_ {s = s} {S₁ = S₁} (lookup ρ (here refl)) (wkᵣ ≫ᵣᵣ ρ) ≡ ρ
  η-lawₛ             : (σ : (s ∷ S₁) ⇛ₛ S₂) → _∷ₛ_ {s = s} {S₁ = S₁} (lookup σ (here refl)) (wkᵣ ≫ᵣₛ σ) ≡ σ
  ...
  -- Distributivity Laws
  distributivityᵣᵣ   : (x : s ∈ S₂) (ρ₁ : S₁ ⇛ᵣ S₂) (ρ₂ : S₂ ⇛ᵣ S₃) → (x ∷ᵣ ρ₁) ≫ᵣᵣ ρ₂ ≡ lookup ρ₂ x ∷ᵣ (ρ₁ ≫ᵣᵣ ρ₂)  
  distributivityᵣₛ   : (x : s ∈ S₂) (ρ₁ : S₁ ⇛ᵣ S₂) (σ₂ : S₂ ⇛ₛ S₃) → (x ∷ᵣ ρ₁) ≫ᵣₛ σ₂ ≡ lookup σ₂ x ∷ₛ (ρ₁ ≫ᵣₛ σ₂)
...
{-# REWRITE lookup-def₁ lookup-def₂ #-}
{-# REWRITE idᵣ-def wkᵣ-def ∷ᵣ-def #-}
{-# REWRITE idₛ-def ∷ₛ-def #-}
...
_∶_⇛ᵣ_ {S₁} {S₂} ρ Γ₁ Γ₂ = ∀ (s : Sort) (x : s ∈ S₁) (T : S₁ ∶⊢ s) → x ∶ T ∈ Γ₁ → (lookup ρ x) ∶ T ⋯ᵣ ρ ∈ Γ₂ 

_∶_⇛ₛ_ : S₁ ⇛ₛ S₂ → Ctx S₁ → Ctx S₂ → Set
_∶_⇛ₛ_ {S₁} {S₂} σ Γ₁ Γ₂ = ∀ (s : Sort) (x : s ∈ S₁) (T : S₁ ∶⊢ s) → x ∶ T ∈ Γ₁ → Γ₂ ⊢ (lookup σ x) ∶ (T ⋯ₛ σ)
...
⊢↑ᵣ {ρ = ρ} {Γ₁ = Γ₁} {Γ₂ = Γ₂} ⊢ρ T _ (there x) _ refl =  ⊢wkᵣ Γ₂ (lookup ρ x) (wk-drop-∈ x (Γ₁ _ x) ⋯ᵣ ρ) (T ⋯ᵣ ρ) (⊢ρ _ x _ refl)
...
⊢↑ₛ {σ = σ} {Γ₁ = Γ₁} {Γ₂ = Γ₂} ⊢σ T _ (there x) _ refl = ⊢wkₛ Γ₂ (lookup σ x) (wk-drop-∈ x (Γ₁ _ x) ⋯ₛ σ) (T ⋯ₛ σ) (⊢σ _ x _ refl)
...
⊢[] : ∀ {Γ : Ctx S} {t : S ⊢ s} {T : S ∶⊢ s} → Γ ⊢ t ∶ T → (t ∷ idₛ) ∶ (Γ ، T) ⇛ₛ Γ
